...
export function VaultOpenScreen() {
  ...
  return (
    <div className="h-screen bg-zinc-950 flex flex-col items-center justify-center text-zinc-100">
      <div className="w-full max-w-sm px-6">
        {/* Logo / title */}
        <div className="text-center mb-10">
          <div className="text-5xl mb-4">📝</div>
          <h1 className="text-3xl font-bold text-white">Notes</h1>
          <p className="text-zinc-400 mt-2 text-sm">Plain Markdown, your files, your sync</p>
        </div>
        ...
        {recent.length > 0 && (
          <div className="mt-8">
            <p className="text-xs text-zinc-500 uppercase tracking-wider mb-3">Recent</p>
            <div className="space-y-1">
              {recent.map((v) => (
                <button
                  ...
                  disabled={loading}
                  className="w-full text-left px-4 py-3 rounded-lg bg-zinc-900 hover:bg-zinc-800 transition-colors disabled:opacity-50"
                >
                  <div className="font-medium text-zinc-200 text-sm">{v.name}</div>
                  <div className="text-xs text-zinc-500 truncate mt-0.5">{v.path}</div>
                </button>
              ))}
            </div>
          </div>
